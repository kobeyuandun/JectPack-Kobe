...
class HttpLoggingInterceptor(tag: String?) : Interceptor {
    ...
    fun setPrintLevel(level: Level){
        if (printLevel == null) {
            throw NullPointerException("printLevel == null. Use Level.NONE instead.")
        }
        printLevel = level
    }
    ...
    private fun logForRequest(request: Request, connection: Connection?) {
        val isLogBody = (printLevel == Level.BODY)
        val isLogHeaders = (printLevel == Level.BODY || printLevel == Level.HEADERS)
    ...
    private fun bodyToString(request: Request) {
        try {
            ...
            val requestBody = build.body ?: return
            val buffer = Buffer()
            requestBody.writeTo(buffer)
            val charset = getCharset(requestBody.contentType())
            log("\nbody:" + JsonUtil.formatJson(charset?.let { buffer.readString(it) }))
        } catch (e: Exception) {
            OkLogger.printStackTrace(e)
        }
    }
